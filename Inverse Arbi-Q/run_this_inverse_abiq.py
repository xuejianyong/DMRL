...
import pandas as pd
import math
from model_inverse_arbiq import MMBRL, Module
import time
import configparser
...
def update():
    ...
    start = time.time()
    filename = "inverse-arbiq-task-samejima_modular_reward-m" + str(N_MODULE) + 's' + str(SIMULATIONS) + 't' + str(TRIAL_SAMEJIMA)
    for simulation_i in range(SIMULATIONS):
        step_list_i = []
        model = MMBRL(n_modules=N_MODULE, trial=TRIAL_SAMEJIMA, method='softmax')
