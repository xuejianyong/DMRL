...
def update():
    ...
    start = time.time()
    filename = "jj-m"+str(N_MODULE)+'s'+str(SIMULATIONS) +'mts'+ str(TRIAL_SAMEJIMA) + 'epsilon-greedy'
    for simulation_i in range(SIMULATIONS):  # SIMULATIONS
        step_list_i = []
        model = MMBRL(n_modules=N_MODULE, trial=TRIAL_SAMEJIMA, method='epsilon-greedy')  # epsilon-greedy greedy stochastic-greedy  softmax
        resp_method = 'jj'  # samejima_modular_reward
