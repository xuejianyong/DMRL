...
this project is used  to generate less performance data for the comparison with jj


"""

# from environment import TextMaze, GraphicMaze
...
# filepath = os.path.abspath(os.path.join(os.getcwd(), '../../app.ini'))
...
def update():
    ...
    filename = "doya-m" + str(N_MODULE) + 's' + str(SIMULATIONS) + 'mts' + str(TRIAL_SAMEJIMA)
    start = time.time()
    for simulation_i in range(SIMULATIONS):
        step_list_i = []
        model = MMBRL(n_modules=N_MODULE, trial=TRIAL_SAMEJIMA, method='epsilon-greedy')  # softmax greedy stochastic-greedy epsilon-greedy
        for trial in range(TRIAL_SAMEJIMA):
            agent_original = [0, 0]
            step = 0
            reward_total = 0
            value_total = 0.0
            ...
            for prey_direction in prey_move_directions:
                ...
                resp_method = 'samejima_modular_reward'  # original  error_softmax  samejima_modular_reward jj
                while True:
                    ...
                    model.learn(str(observation), action, reward, str(observation_), resp_method)
        ...
        # for module_i in model.modules:
        #     print(module_i.state_values['[0, 0]'])
