...
class MMBRL:
    def __init__(self, n_modules, trial, method='epsilon-greedy'):
        ...
        self.watch_list = []
        self.watch_limit = 10
        self.eta = 0.5  # eligibility traces
    ...
    def learn(self, s, a, r, s_, resp_method, step):
        key = s + str(a)
        ...
        if resp_method == 'original':
            ...
            for module_i in self.modules:
                ...
                upper_predict_probability = module_i.responsibility_predictor/sum_responsibility_predictor * predict_probability
            ...
            resp_predictor_list = []
            for module_i in self.modules:
                resp_predictor_list.append(module_i.responsibility_predictor)
            sum_predictor = sum(resp_predictor_list)
            for module_i in self.modules:
                sum_probability_i = sum(module_i.state_action_state_pro[key].values())
        ...
        if step > 0:
            posterior = 0.0
            for module_i in self.modules:
                posterior += (module_i.responsibility - module_i.responsibility_previous) * module_i.state_values[s_]
            posterior *= self.gamma

            resp_difference_list = []
            for module_i in self.modules:
                if module_i.responsibility < module_i.responsibility_previous:
                    resp_difference_list.append(module_i.responsibility_previous)
                else:
                    resp_difference_list.append(0)
            """
            if sum(resp_difference_list) == 0:
                for module_i in self.modules:
                ...
                resp_difference_list = np.array(resp_difference_list) / sum(np.square(resp_difference_list))
                for module_i, resp_pre in zip(self.modules, resp_difference_list):
                    # update state predictor
                    module_i.state_action_state_pro[key][s_] += module_i.responsibility * (1 - self.ksai * module_i.state_action_state_pro[key][s_])
                    imaginary_reward = resp_pre * posterior
                    reward_error = r + imaginary_reward - module_i.state_action_reward_value[key]
                    module_i.state_action_reward_value[key] += module_i.responsibility * reward_error * self.lr
                    # calculating eligibility traces
                    module_i.eligibility[s] *= self.gamma * self.eta
                    module_i.eligibility[s] += 1
                    # calculating td-error
                    td_error = r + imaginary_reward + self.gamma * module_i.state_values[s_] - module_i.state_values[s]
                    module_i.state_values[s] += module_i.responsibility * td_error * self.lr * module_i.eligibility[s]
            ...
            """
            """
            key_previous = self.state_previous + str(self.action_previous)
            if sum(resp_difference_list) == 0:
                for module_i in self.modules:
                    modular_reward_i = self.reward_previous + posterior
                    reward_error_i = modular_reward_i - module_i.state_action_reward_value[key_previous]
                    # update reward model
                    module_i.state_action_reward_value[key_previous] += module_i.responsibility_previous * reward_error_i * self.lr

                    # update state value function
                    # module_i.eligibility[self.state_previous] *= self.gamma * self.eta
                    # module_i.eligibility[self.state_previous] += 1
                    module_i.eligibility[self.state_previous] = (1 - self.eta) * module_i.eligibility[self.state_previous] +\
                                                                            self.eta * module_i.responsibility_previous
                    # td_error = modular_reward_i + self.gamma * module_i.state_values[s] - module_i.state_values[self.state_previous]
                    td_error = module_i.responsibility_previous * state_error
                    module_i.state_values[self.state_previous] += td_error * self.lr * \
                                                                  module_i.eligibility[self.state_previous]

                    # td_error_i = modular_reward_i + self.gamma * module_i.state_values[s] - module_i.state_values[self.state_previous]
                    # td_error_i = module_i.responsibility_previous * state_error
                    # module_i.state_values[self.state_previous] += module_i.responsibility_previous * td_error_i * self.lr
                    # module_i.eligibility[s] = (1 - self.eta) * module_i.eligibility[self.state_previous] +\
                    #                                  self.eta * module_i.responsibility_previous
                    # td_error_i = module_i.responsibility_previous * state_error
                    # module_i.state_values[self.state_previous] += self.lr * td_error_i * module_i.eligibility[s]
            else:
                resp_difference_list = np.array(resp_difference_list) / sum(np.square(resp_difference_list))
                for module_i, resp_pre in zip(self.modules, resp_difference_list):
                    modular_reward_i = self.reward_previous + posterior
                    ...
                    # td_error = modular_reward_i + self.gamma * module_i.state_values[s] - module_i.state_values[self.state_previous]
                    td_error = module_i.responsibility_previous * state_error
                    module_i.state_values[self.state_previous] += td_error * self.lr * module_i.eligibility[self.state_previous]

                    # td_error_i = modular_reward_i + self.gamma * module_i.state_values[s] - module_i.state_values[self.state_previous]
                    # td_error_i = module_i.responsibility_previous * state_error
                    # module_i.state_values[self.state_previous] += module_i.responsibility_previous * td_error_i * self.lr
                    # td_error_i = module_i.responsibility_previous * state_error
                    # module_i.state_values[module_i.state_previous] += module_i.responsibility_previous * td_error_i * self.lr
                    # module_i.eligibility[s] = (1 - self.eta) * module_i.eligibility[self.state_previous] + \
                    #                           self.eta * module_i.responsibility_previous
                    # td_error_i = module_i.responsibility_previous * state_error
                    # module_i.state_values[self.state_previous] += self.lr * td_error_i * module_i.eligibility[s]
            ...
            for module_i in self.modules:
                ...
                module_i.state_action_state_pro[key_previous][s] += module_i.responsibility_previous * \
                                                                    (1 - self.ksai * module_i.state_action_state_pro[key_previous][s])
                # update state-action value in the q_table
                module_i.update_state_actions(self.state_previous, self.action_previous)
                action_count_dict = module_i.state_action_count[self.state_previous]  # get the action list in this state
                for action_i in action_count_dict.keys():
                    state_action_str = self.state_previous + str(action_i)
                    latter_value = 0.0
                    state_action_state_dict = module_i.state_action_state_pro[state_action_str]
                    sum_probability = sum((state_action_state_dict.values()))
                    for next_state_i in state_action_state_dict.keys():
                        latter_value += state_action_state_dict[next_state_i] / sum_probability * module_i.state_values[next_state_i]
                    module_i.q_table.loc[self.state_previous, action_i] = \
                        module_i.state_action_reward_value[state_action_str] + self.gamma * latter_value
            """

            # calculating the error of entire task
            state_error = r + self.gamma * self.state_values_archiecture[s_] - self.state_values_archiecture[s]
            self.state_values_archiecture[s_] += state_error * self.lr
            for module_i in self.modules:
                # update state predictor
                # module_i.state_action_state_pro[key][s_] += module_i.responsibility * (1 - self.ksai * module_i.state_action_state_pro[key][s_])
                sum_probability_i = sum(module_i.state_action_state_pro[key].values())
                error_i = sum_probability_i - module_i.state_action_state_pro[key][s_]
                module_i.state_action_state_pro[key][s_] += module_i.responsibility * error_i * self.lr
                """
                sum_probability_i = sum(module_i.state_action_state_pro[key].values())
                ...
                # imaginary_reward = (module_i.responsibility - module_i.responsibility_previous) * module_i.state_values[s_]
                imaginary_reward = 0
                reward_error = r - module_i.state_action_reward_value[key]
                module_i.state_action_reward_value[key] += module_i.responsibility * reward_error * self.lr
                ...
                module_i.eligibility[s] *= self.gamma * self.eta
                module_i.eligibility[s] += 1
                # module_i.eligibility[s] = (1 - self.eta) * module_i.eligibility[s] + self.eta * module_i.responsibility
                ...
                for action_i in action_count_dict.keys():
                    ...
                    sum_probability = sum((state_action_state_dict.values()))
                    for next_state_i in state_action_state_dict.keys():
                        latter_value += state_action_state_dict[next_state_i] / sum_probability * module_i.state_values[
                            next_state_i]
                    module_i.q_table.loc[s, action_i] = module_i.state_action_reward_value[state_action_str] + self.gamma * latter_value

        self.state_previous = s
        self.reward_previous = r
        self.action_previous = a
        for module_i in self.modules:
            module_i.responsibility_previous = module_i.responsibility
        ...
        for module_i in self.modules:
            ...
            # td_error = r + self.gamma * module_i.state_values[s_] - module_i.state_values[s]
            # td_error = state_error
            # module_i.state_values[s] += module_i.responsibility * td_error * self.lr
...
class Module:
    def __init__(self, serial, actions, n_modules):
        ...
        self.responsibility_previous = 1

        self.responsibility = 1
        self.responsibility_predictor = 1
